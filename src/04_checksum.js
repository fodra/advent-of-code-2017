...
function checksum(arr) {
  let sum = 0;
  arr.forEach( (row, i) => {
    let sortedRow = row.sort((a, b) => {
      return a - b;
    });

    let findPair = ((sortedArr) => {
      for(let i = 0; i < sortedArr.length; i++) {
        for(let j = i + 1; j < sortedArr.length; j++) {
          if (sortedArr[j] % sortedArr[i] === 0) {
            return { a: sortedArr[i], b: sortedArr[j] };
          }
        }
      }
      return {a: 0, b: 0};
    });
    let result = findPair(sortedRow);
    sum += (result.b / result.a);
  });
  return sum;
}
...
(() => {
    console.log("Day three - Advent of Code 2017 - Checksum");
    let input = [
